...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale en-us /uninstall /silent /vendor "Vendor Name" /productName "SampleClassLibrary Documentation" /mediaBookList "SampleClassLibrary Documentation" > NUL
...
REM build time.
IF EXIST "SampleClassLibraryDocumentation.msha" COPY /Y "SampleClassLibraryDocumentation.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation uninstall /vendor "Vendor Name" /productName "SampleClassLibrary Documentation" /bookList "SampleClassLibrary Documentation" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation install /sourceUri "%CD%\SampleClassLibraryDocumentation.msha"
