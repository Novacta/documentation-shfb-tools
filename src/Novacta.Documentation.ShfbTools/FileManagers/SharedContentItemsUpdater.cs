...
namespace Novacta.Documentation.ShfbTools.FileManagers
{
    ...
    class SharedContentItemsUpdater : EditFileManager
    {
        ...
        /// having null or empty elements.</exception>
            ...
            IEnumerable<(string Id, string InnerText)> items) : base(path)
        {
            ...
            foreach (var item in items)
            {
                if ((null == item.Id)
                    || (null == item.InnerText)
                    || (String.Empty == item.Id)
                    || (String.Empty == item.InnerText))
                {
        ...
        protected override void OnCommit()
        {
            ...
            foreach (var item in this.items)
            {
                ...
                contentNode.SelectSingleNode("item[@id='" + item.Id + "']");
                ...
                idAttribute.Value = item.Id;
                targetItemNode.Attributes.Append(idAttribute);

                targetItemNode.InnerText = item.InnerText;
            }
